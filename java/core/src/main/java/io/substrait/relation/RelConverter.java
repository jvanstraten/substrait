...
public class RelConverter implements RelVisitor<Rel, RuntimeException> {
  ...
  private List<io.substrait.proto.Expression>
  toProto(Collection<Expression> expressions) {
  ...
  public Rel visit(Aggregate aggregate) throws RuntimeException {
    var builder =
        AggregateRel.newBuilder()
            ...
            .addAllGroupings(
                aggregate.getGroupings().stream().map(this::toProto).toList())
            .addAllMeasures(
                aggregate.getMeasures().stream().map(this::toProto).toList());
  ...
  private AggregateRel.Measure toProto(Aggregate.Measure measure) {
    var func = AggregateFunction.newBuilder()
                   ...
                   .setFunctionReference(functionLookup.getFunctionReference(
  ...
  public Rel visit(NamedScan namedScan) throws RuntimeException {
    return Rel.newBuilder()
        .setRead(ReadRel.newBuilder()
                     ...
                     .setNamedTable(ReadRel.NamedTable.newBuilder().addAllNames(
                         namedScan.getNames()))
  ...
  public Rel visit(Project project) throws RuntimeException {
    var builder =
        ProjectRel.newBuilder()
            ...
            .addAllExpressions(
                project.getExpressions().stream().map(this::toProto).toList());
            ...
            ReadRel.newBuilder()
                ...
                .setVirtualTable(
                    ReadRel.VirtualTable.newBuilder()
                        .addAllValues(virtualTableScan.getRows()
                                          .stream()
                                          .map(this::toProto)
  ...
  private RelCommon common(io.substrait.relation.Rel rel) {
    ...
    rel.getRemap().ifPresentOrElse(
        r
        -> builder.setEmit(
            RelCommon.Emit.newBuilder().addAllOutputMapping(r.indices())),

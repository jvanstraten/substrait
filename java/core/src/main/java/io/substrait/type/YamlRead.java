...
public class YamlRead {
  static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(YamlRead.class);

  private final static List<String> FUNCTIONS = Collections.unmodifiableList(
      Arrays.asList("boolean", "aggregate_generic", "arithmetic_decimal",
                    "arithmetic", "comparison", "datetime", "string"));
  public static void main(String[] args) throws Exception {
    try {
      System.out.println("Read: " +
                         YamlRead.class.getResource("/functions_boolean.yaml"));
  ...
  public static List<SimpleExtension.Function> loadFunctions() {
    return loadFunctions(
        FUNCTIONS.stream()
            .map(c
                 -> String.format("/src/substrait/extensions/functions_%s.yaml",
                                  c))
            .toList());
  }

  public static List<SimpleExtension.Function>
  loadFunctions(List<String> files) {
  ...
  private static Stream<SimpleExtension.Function> parse(String name) {
    ...
    try {
      ...
      var doc = mapper.readValue(new File(name),
                                 SimpleExtension.FunctionSignatures.class);

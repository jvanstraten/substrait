...
public class ParseToPojo {
  ...
  parameterizedType(SubstraitTypeParser.StartContext ctx) {
    return (ParameterizedType)ctx.accept(Visitor.PARAMETERIZED);
  }

  public static TypeExpression
  typeExpression(SubstraitTypeParser.StartContext ctx) {
  ...
  public static enum Visitor implements SubstraitTypeVisitor<TypeExpression> {
    ...
    visitStart(final SubstraitTypeParser.StartContext ctx) {
    ...
    @Override
    public TypeExpression
    visitTypeLiteral(final SubstraitTypeParser.TypeLiteralContext ctx) {
    ...
    @Override
    public Type
    visitTimestampTz(final SubstraitTypeParser.TimestampTzContext ctx) {
    ...
    @Override
    public Type
    visitIntervalDay(final SubstraitTypeParser.IntervalDayContext ctx) {
      return withNull(ctx).INTERVAL_DAY;
    }

    @Override
    public Type
    visitIntervalYear(final SubstraitTypeParser.IntervalYearContext ctx) {
    ...
    @Override
    public TypeExpression
    visitFixedChar(final SubstraitTypeParser.FixedCharContext ctx) {
      return of(ctx.len, withNull(ctx)::fixedChar, withNullP(ctx)::fixedCharE,
                withNullE(ctx)::fixedCharE);
    }

    private TypeExpression
    of(SubstraitTypeParser.NumericParameterContext ctx,
    ...
    @Override
    public TypeExpression
    visitVarChar(final SubstraitTypeParser.VarCharContext ctx) {
      return of(ctx.len, withNull(ctx)::varChar, withNullP(ctx)::varCharE,
                withNullE(ctx)::varCharE);
    }

    @Override
    public TypeExpression
    visitFixedBinary(final SubstraitTypeParser.FixedBinaryContext ctx) {
      return of(ctx.len, withNull(ctx)::fixedBinary,
                withNullP(ctx)::fixedBinaryE, withNullE(ctx)::fixedBinaryE);
    }

    @Override
    public TypeExpression
    visitDecimal(final SubstraitTypeParser.DecimalContext ctx) {
      ...
      checkExpression();
      return withNullE(ctx).decimalE(ctx.precision.accept(this),
                                     ctx.scale.accept(this));
    }
    ...
    @Override
    public TypeExpression
    visitStruct(final SubstraitTypeParser.StructContext ctx) {
      var types = ctx.expr().stream().map(t -> t.accept(this)).toList();
      if (types.stream().allMatch(t -> t instanceof Type)) {
        return withNull(ctx).struct(
            types.stream().map(t -> ((Type)t)).toList());
      }

      if (types.stream().allMatch(t -> t instanceof ParameterizedType)) {
        checkParameterizedOrExpression();
        return withNullP(ctx).structE(
            types.stream().map(t -> ((ParameterizedType)t)).toList());
      }
    ...
    @Override
    public TypeExpression
    visitNStruct(final SubstraitTypeParser.NStructContext ctx) {
      throw new UnsupportedOperationException();
    }
    ...
    public TypeExpression visitMap(final SubstraitTypeParser.MapContext ctx) {
      ...
      if (key instanceof ParameterizedType && value instanceof
                                                  ParameterizedType) {
        checkParameterizedOrExpression();
        return withNullP(ctx).mapE((ParameterizedType)key,
    ...
    private TypeCreator
    withNull(SubstraitTypeParser.RequiredTypeContext required) {
      return Type.withNullability(
          ((SubstraitTypeParser.TypeContext)required.parent).isnull != null);
    }

    private TypeExpressionCreator
    withNullE(SubstraitTypeParser.RequiredTypeContext required) {
      return TypeExpression.withNullability(
          ((SubstraitTypeParser.TypeContext)required.parent).isnull != null);
    }

    private ParameterizedTypeCreator
    withNullP(SubstraitTypeParser.RequiredTypeContext required) {
    ...
    @Override
    public TypeExpression
    visitTypeParam(final SubstraitTypeParser.TypeParamContext ctx) {
      checkParameterizedOrExpression();
    ...
    @Override
    public TypeExpression
    visitParenExpression(final SubstraitTypeParser.ParenExpressionContext ctx) {
      return ctx.expr().accept(this);
    }

    @Override
    public TypeExpression
    visitIfExpr(final SubstraitTypeParser.IfExprContext ctx) {
      checkExpression();
    ...
    @Override
    public TypeExpression
    visitTernary(final SubstraitTypeParser.TernaryContext ctx) {
      checkExpression();
      return TypeExpression.IfOperation.builder()
          .ifCondition(ctx.ifExpr.accept(this))
          .thenExpr(ctx.thenExpr.accept(this))
    ...
    @Override
    public TypeExpression
    visitBinaryExpr(final SubstraitTypeParser.BinaryExprContext ctx) {
      checkExpression();

...
abstract class BaseProtoTypes<T, I> {
  ...
  public BaseProtoTypes(Type.Nullability nullability) {
    ...
    BOOLEAN =
        wrap(Type.Boolean.newBuilder().setNullability(nullability).build());
    ...
    TIMESTAMP =
        wrap(Type.Timestamp.newBuilder().setNullability(nullability).build());
    TIMESTAMP_TZ =
        wrap(Type.TimestampTZ.newBuilder().setNullability(nullability).build());
    ...
    INTERVAL_DAY =
        wrap(Type.IntervalDay.newBuilder().setNullability(nullability).build());
    INTERVAL_YEAR = wrap(
        Type.IntervalYear.newBuilder().setNullability(nullability).build());
    UUID = wrap(Type.UUID.newBuilder().setNullability(nullability).build());
  }

  public abstract T fixedChar(I len);

  public final T fixedChar(int len) { return fixedChar(i(len)); }

  public final T fixedChar(String len) { return fixedChar(integerParam(len)); }

  public final T varChar(int len) { return varChar(i(len)); }

  public final T varChar(String len) { return varChar(integerParam(len)); }

  public final T fixedBinary(int len) { return fixedBinary(i(len)); }
  ...
  public final T struct(T... types) { return struct(Arrays.asList(types)); }

...
public class FunctionLookup {
  static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(FunctionLookup.class);

  private final BidiMap<Integer, SimpleExtension.FunctionAnchor> map =
      new BidiMap<>();
  ...
  public SimpleExtension.ScalarFunctionVariant
  getScalarFunction(int reference,
  ...
  public SimpleExtension.AggregateFunctionVariant
  getAggregateFunction(int reference,
                       SimpleExtension.ExtensionCollection extensions) {
    var anchor = map.get(reference);
    if (anchor == null) {
      throw new IllegalArgumentException(
  ...
  public void addFunctionsToPlan(Plan.Builder builder) {
    ...
    for (var e : map.forwardMap.entrySet()) {
      SimpleExtensionURI uri = uris.computeIfAbsent(
  ...
  private static class BidiMap<T1, T2> {
    ...
    public T2 get(T1 t1) { return forwardMap.get(t1); }

    public T1 reverseGet(T2 t2) { return reverseMap.get(t2); }

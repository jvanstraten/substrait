...
public class FieldSelectionConverter implements CallConverter {
  ...
  public FieldSelectionConverter() { super(); }
  ...
  convert(RexCall call, Function<RexNode, Expression> topLevelConverter) {
    ...
    if (reference.getKind() != SqlKind.LITERAL ||
        !(reference instanceof RexLiteral)) {
      logger.warn(
          "Found item operator without literal kind/type. This isn't handled well. Reference was {} with toString {}.",
          reference.getKind().name(), reference);
      return Optional.empty();
    }
    ...
    case ROW: {
      ...
      if (input instanceof FieldReference) {
        return Optional.of(
            ((FieldReference)input).dereferenceStruct(index.get()));
      } else {
        return Optional.of(
            FieldReference.newStructReference(index.get(), input));
      }
    }
    case ARRAY: {
      var index = toInt(literal);
      ...
      if (input instanceof FieldReference) {
        return Optional.of(
            ((FieldReference)input).dereferenceList(index.get()));
      } else {
    ...
    case MAP: {
      ...
      Expression.Literal keyLiteral =
          ExpressionCreator.string(false, mapKey.get());
      if (input instanceof FieldReference) {

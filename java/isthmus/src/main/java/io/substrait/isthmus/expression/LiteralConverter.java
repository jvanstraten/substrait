...
public class LiteralConverter {
  static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(LiteralConverter.class);

  private static final long MICROS_IN_DAY = TimeUnit.DAYS.toMicros(1);

  static final DateTimeFormatter CALCITE_LOCAL_DATE_FORMATTER =
  ...
  private Expression nullOf(RexLiteral literal) { return null; }
  ...
  public static Expression.Literal convert(RexLiteral literal) {
    ...
    case TINYINT -> i8(n, i(literal).intValue());
        ...
        case BOOLEAN -> bool(n, (Boolean)literal.getValue()); case CHAR -> {
          var val = literal.getValue(); yield switch (val) {
            case NlsString nls -> fixedChar(n, nls.getValue());
            default
            -> throw new UnsupportedOperationException(
                "Unable to handle char type: " + val);
          };
        ...
        -> {
          ...
          yield decimal(n, bd, literal.getType().getPrecision(),
                        literal.getType().getScale());
        } case VARCHAR
        -> {
          if (literal.getType().getPrecision() ==
        ...
        -> fixedBinary(
            n, ByteString.copyFrom(padRightIfNeeded(
                   literal.getValueAs(
                       org.apache.calcite.avatica.util.ByteString.class),
                   literal.getType().getPrecision())));
        case VARBINARY
        -> binary(n, ByteString.copyFrom(literal.getValueAs(byte[].class)));
        case SYMBOL
        -> {
          Object value = literal.getValue(); yield switch (value) {
            case NlsString s -> string(n, s.getValue());
            case TimeUnitRange tur -> string(n, tur.name());
            default
            -> throw new UnsupportedOperationException(
                "Unable to handle symbol: " + value);
          };
        } case DATE
        -> {
        ...
        -> {
          ...
          LocalDateTime ldt = LocalDateTime.parse(
              timestamp.toString(), CALCITE_LOCAL_DATETIME_FORMATTER);
        ...
        -> {
          throw new UnsupportedOperationException(
              "Need to implement IntervalYear");
        }

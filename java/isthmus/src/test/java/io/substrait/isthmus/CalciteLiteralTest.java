...
public class CalciteLiteralTest extends CalciteObjs {
  ...
  @Test
  void nullLiteral() {
    test(typedNull(Type.NULLABLE.varChar(10)),
  ...
  void tBinary() {
    ...
    test(binary(false, val),
         c(new org.apache.calcite.avatica.util.ByteString(val),
           SqlTypeName.VARBINARY));
  }

  @Test
  void tTime() {
    test(time(false, (14L * 60 * 60 + 22 * 60 + 47) * 1000 * 1000),
         rex.makeTimeLiteral(new TimeString(14, 22, 47), 6));
  }

  @Test
  void tDate() {
    test(date(false, (int)LocalDate.of(2002, 2, 14).toEpochDay()),
  ...
  @Test
  void tTimestampTZ() {
    // Calcite has TimestampWithTimeZoneString but it doesn't appear to be
    // available as a literal or data type. (Doesn't exist in SqlTypeName.)
  }
  ...
  @Test
  void tMap() {
    var ss = ImmutableMap.<Expression.Literal, Expression.Literal>of(
        string(false, "foo"), i32(false, 4), string(false, "bar"),
        i32(false, -1));
    var calcite = rex.makeLiteral(
  ...
  void tList() {
    test(list(false, i32(false, 4), i32(false, -1)),
         rex.makeLiteral(Arrays.asList(4, -1),
                         type.createArrayType(t(SqlTypeName.INTEGER), -1),
                         false, false));
  }
